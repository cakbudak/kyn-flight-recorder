...
| Knowledge import reads host files or fetches a hostile URL | API accepts bounded UTF-8 content only; SmartRead resolves same-workspace source-version IDs and never accepts a path or URL | imported text remains untrusted model context |
| One BoardRoom member anchors on peers | members receive only the common mapped input and execute before the join in separate operation sessions | provider-side infrastructure is outside the isolation claim |
| Editor invents consensus or hides dissent | code derives counts, quorum, failures, convergence, and dissenting IDs; raw member Steps and barrier event remain immutable beside editor output | editor prose may still be poor, but cannot change the recorded barrier or route authority |
| Parallel member mints an effect or pauses the room | publication rejects approval/effect-capable members and nested fan-out; writes and approvals are explicit downstream nodes | external Actions remain intentionally absent from this public cut |
| Quarantined or rejected proposal contaminates recall | search joins only active Human-promoted Memory versions; qualification and exact-fingerprint decision precede activation | a Human can still promote a weak but correctly sourced observation |
